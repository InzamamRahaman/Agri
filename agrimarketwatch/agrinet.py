from flask import Flask, request, jsonify, make_response, url_for,render_template, flash, redirect
...
def lineCharts():
    crops = mongo.db.monthly.distinct("commodity")
    trimmed_crops = dict([(crop, crop.replace(" ", "").replace("(","").replace(")","")) for crop in crops])
    return render_template("LineCharts.html",crops=trimmed_crops)

@app.route("/stackedBarCharts")
def barCharts():
    crops = mongo.db.monthly.distinct("commodity")
    trimmed_crops = dict([(crop, crop.replace(" ", "").replace("(","").replace(")","")) for crop in crops])
    return render_template("stackedBarCharts.html",crops=trimmed_crops)
...
@app.route('/crops/monthly/commodity/<comm>')
def monthly_crop_commodity(comm = None):
	res = []
	options = {}
	offset = 0
	limit = 100
